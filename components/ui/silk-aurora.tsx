...
export function SilkAurora({
  ...
  React.useEffect(() => {
    ...
    // Read live rather than once at mount: toggling the OS setting used to
    // leave the canvas running until a full remount.
    ...
    // A 60fps WebGL loop has no business running while the canvas is scrolled
    ...
    const isAwake = () => onScreen && document.visibilityState === "visible";
    ...
    /** Undoes everything set up above. Every bail-out path returns this. */
    const teardownBase = () => {
    ...
    try {
      ...
      // These never change for the life of the effect - `settings` is a
      // dependency, so a prop change tears the context down and rebuilds it.
      ...
      // Accumulated animation time rather than wall-clock since mount. Only
      // advancing it while the canvas is awake stops the ribbons teleporting
      // when you scroll back to the hero or return to the tab.
      let clock = 0;
